...
def launcher_from_handle(handle: str, log_fn: Optional[Callable] = None) -> JobLauncher:
    """Return the launcher that owns ``handle`` (``local:{pid}`` /
...
def select_launcher(backend: Optional[str], *, slurm_ok: bool) -> JobLauncher:
    ...
    SkyPilot dispatch is not handled here — it needs config loading, which the
    webapp's ``orchestrator_launcher_for`` owns. Raise on anything we don't handle
    rather than silently returning a LocalJobLauncher, so a future caller that
    skips that guard can't quietly run a skypilot orchestrator on the
    control-plane host."""
    ...
    raise ValueError(
        ...
        "skypilot launch is owned by the webapp's orchestrator_launcher_for"
    )
...
__all__ = [
    ...
    "ACTIVE_STATUSES",
    "LocalJobLauncher", "SlurmJobLauncher", "SkyPilotVmJobLauncher",
