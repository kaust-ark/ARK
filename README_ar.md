...
يستخدم ARK نموذج إعدادات من ثلاثة مستويات للحوسبة السحابية:
1. **افتراضيات النظام**: تُعين في `webapp.env` (مثل `CLOUD_REGION`, `CLOUD_NETWORK`).
2. **افتراضيات المستخدم العامة**: تُعين في لوحة **الإعدادات** (⚙️). وتطبق على جميع مشاريعك.
3. **تجاوزات المشروع**: تُعين أثناء إنشاء المشروع أو إعادة تشغيله. ولها الأولوية القصوى.

يسمح لك هذا التسلسل بتحديد نوع الجهاز القياسي وإعدادات VPC مرة واحدة، مع سهولة التبديل إلى نسخة GPU قوية لتجربة محددة عالية الكثافة.
