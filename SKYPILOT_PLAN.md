...
| **PR2** | **Layer 1 `SkyPilotBackend`** — experiments as a `sky` task: `setup()` launches a cluster, code via `file_mounts`, `wait_for_completion` via task status/marker, `sync_from_backend`, `teardown` → `sky down`. ⚠️ The `skypilot` pytest marker + CI `-m "…and not skypilot"` filter added in PR1 are **inert until a test actually carries `@pytest.mark.skypilot`** — any real-provisioning test added here (or in PR3) MUST be marked, or it will run in CI. | ✅ mock sky SDK; optional local-k8s integration test (`skypilot` mark) |
