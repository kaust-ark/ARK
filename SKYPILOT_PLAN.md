...
| **PR4** | **Go-live.** Orchestrator image strategy (`setup:` block → baked image), secret injection + teardown wipe (incl. Gemini/Claude OAuth **session** file-mounts, which PR3 does not provision — API-key auth only), `sky autostop --down`/spot for cost safety (**required** for experiment clusters — see the box in §3, no cross-plane teardown exists), map the intervention gate. | partial |
