# idea2paper — المهام والمشكلات المعروفة
...
- تكامل بدون كود: انسخ المهارات إلى `~/.claude/skills/`، وكلاء idea2paper يكتشفونها تلقائياً
- الاستراتيجية: لا تثبّت كل ١٧٠+، اختر حزماً لكل مجال لتجنب تضخم الرموز
- أضف قسم توصية مهارات المجال إلى وثائق idea2paper
...
- مفقود: التحقق من تكامل خلفية الحوسبة (Slurm، SkyPilot)
...
### [ ] SkyPilot حوسبة سحابية — تحقق من طرف إلى طرف
- SkyPilot الآن هو المسار السحابي الوحيد (أُزيلت خلفية AWS/GCP/Azure الأصلية)؛ تصميم عزل مساحة العمل لكل مستخدم + حساب خدمة launcher المركزي جديد ويحتاج تحققاً على مشاريع حقيقية وعلى نطاق واسع (انظر `SKYPILOT_PLAN.md`، `docs/SKYPILOT_TEST.md`)
- يحتاج اختبار: توفير عنقود المنسق + عنقود التجارب، أنواع مثيلات GPU، عند الطلب مقابل المزايدة
- يحتاج اختبار: تدفق منح IAM متعدد المستأجرين والإطلاق عبر المشاريع إلى مشروع GCP لكل مستخدم
- يحتاج اختبار: دقة تتبع التكلفة والإنهاء التلقائي (`autostop`) بعد اكتمال التشغيل
